...
//  %type 定义非终结符的语义值类型
%type  <ptr> program ExtDefList ExtDef  Specifier ExtDecList FuncDec CompSt VarList VarDec ParamDec Stmt StmList DefList Def DecList Dec Exp Args DimensionList
...
%token EXT_DEF_LIST EXT_VAR_DEF FUNC_DEF FUNC_DEC EXT_DEC_LIST PARAM_LIST PARAM_DEC VAR_DEF DEC_LIST DEF_LIST COMP_STM STM_LIST EXP_STMT IF_THEN IF_THEN_ELSE DPLUS_BEFORE DPLUS_AFTER DMINUS_BEFORE DMINUS_AFTER DIMENSION DIMENSION_LIST
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(2,EXT_VAR_DEF,yylineno,$1,$2);}   //该结点对应一个外部变量声明
         ...
         | error SEMI   {$$=NULL;}
         ;
Specifier:  TYPE    {$$=mknode(0,TYPE,yylineno);strcpy($$->type_id,$1);if($$->type=!strcmp($1,"int"))$$->type=INT;if(!strcmp($1, "float"))$$->type=FLOAT;if(!strcmp($1, "char"))$$->type=CHAR;}
           ;      
...
VarDec:  ID          {$$=mknode(0,ID,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
        | ID DimensionList {$$=mknode(1, DIMENSION, yylineno, $2);strcpy($$->type_id, $1);} // 数组声明
         ;
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      ...
      | DPLUS  Exp      {$$=mknode(1,DPLUS_AFTER,yylineno,$2);strcpy($$->type_id,"DPLUS_AFTER");}
      | Exp DPLUS      {$$=mknode(1,DPLUS_BEFORE,yylineno,$1);strcpy($$->type_id,"DPLUS_BEFORE");}
      | DMINUS Exp    {$$=mknode(1,DMINUS_AFTER, yylineno,$2);strcpy($$->type_id,"DMINUS_AFTER");}
      | Exp DMINUS    {$$=mknode(1,DMINUS_BEFORE,yylineno,$1);strcpy($$->type_id,"DMINUS_BEFORE");}
